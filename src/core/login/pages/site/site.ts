...
export class CoreLoginSitePage {
    ...
    constructor(navParams: NavParams,
        ...
        this.sitesProvider.checkSite('http://elearning.vku.udn.vn')
            .catch((error: any) => {
                // Attempt guessing the domain if the initial check failed
                const domain = CoreUrl.guessMoodleDomain(url);

                if (domain && domain != url) {
                    return this.sitesProvider.checkSite(domain).catch((secondError: any) => {
            ...
            .catch((error: any) => this.showLoginIssue(url, error))
        ...
        if (this.loginHelper.hasSeveralFixedSites()) {
            ...
            CoreConfig.instance.get(CoreLoginHelperProvider.ONBOARDING_DONE, false).then((onboardingDone: any) => {
                if (!onboardingDone) {
                    // Check onboarding.
                    this.showOnboarding();
                }
